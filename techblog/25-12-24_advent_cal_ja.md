...
プロバビリスティック・プログラミングは、不確実性の下で推論を行うモデルを構築するための強力なパラダイムとして台頭してきました。複雑な推論アルゴリズムを手作業でコーディングする代わりに、実務者はモデルをプログラムとして表現し、数学的な重労働をシステムに任せることができます。しかし、ここに課題があります：汎用的な推論は往々にして遅すぎる一方で、手作業でチューニングしたアルゴリズムは実装と保守が煩雑です。
...
- **ハイブリッド推論**：ニューラルネットワークの提案分布と原理的なベイズ推論（MCMC、SMC、変分推論）を単一のワークフローでシームレスに組み合わせ
- **確率的構造**：潜在変数の数が未知である問題（例：物体検出、未知のKを持つクラスタリング）をモデル化
- **プログラマブルなトレードオフ**：プロトタイピングには高レベルの抽象化を使用し、本番環境では最適化されたコードにドロップダウン
- **クリーンなAPI**：コンパイラの内部構造と格闘することなく、自動微分と推論プリミティブを利用
...
# これで以下のような質問に答えられます：
# - 居住面積が価格に与える影響の95%信頼区間は？
# - 品質がサイズより重要であることにどれだけ確信があるか？
# - 新しい住宅の妥当な価格の範囲は？
```
...
不動産データにはしばしば外れ値（差し押さえ、高級住宅の販売）があります。GenJAXでは、確率的分岐を使ってモデルを容易に拡張できます：
...
ここがGenのプログラマブル推論が輝くところです—離散的な外れ値インジケータにはギブスサンプリングを、連続的な係数にはHMCを混合できます。
...
科学研究のためのベイズモデルを構築する場合でも、不確実性の下で推論する必要がある知覚システムを開発する場合でも、GenJAXは柔軟性と自動化の間の魅力的な中間点を提供します。
